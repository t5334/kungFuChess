...
def test_log_move_with_capture():
    ...
    assert logger.get_moves("black") == ["N: g8->f6 x"]
    assert logger.get_points("black") == 0  # Points are handled by ScoreBoard now
...
def test_multiple_moves():
    ...
    assert logger.get_moves("white") == ["B: c1->g5", "Q: d1->h5 x"]
    assert logger.get_moves("black") == ["P: e7->e5", "K: e8->e7"]
    assert logger.get_points("white") == 0  # Points handled by ScoreBoard
