...
class Board:
    ...
    def m_to_pix(self, pos_m: tuple[float, float]) -> tuple[int, int]:
        ...
        # return x_px, y_px
        x_m, y_m = pos_m
        x_px = int(round(x_m / self.cell_W_m * self.cell_W_pix)) + self.offset_x
        y_px = int(round(y_m / self.cell_H_m * self.cell_H_pix)) + self.offset_y
        return x_px, y_px
    
    def cell_to_pix(self, cell: tuple[int, int]) -> tuple[int, int]:
        """Convert a cell *(row, col)* to pixel position on the full image, including offset."""
        r, c = cell
        x = self.offset_x + c * self.cell_W_pix
        y = self.offset_y + r * self.cell_H_pix
        return x, y
